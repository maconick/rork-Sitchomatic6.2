...
struct ConcurrencyCapSelector: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
                ...
                ForEach(presets, id: \.value) { preset in
                    Button {
                        ...
                        Text(preset.label)
                            ...
                            .padding(.vertical, 12)
                            .background(engine.maxCap == preset.value ? Color.cyan : Color(.tertiarySystemFill))
            ...
            Text("AI starts at 1 and ramps up to this cap based on live conditions")
...
struct AdaptiveConcurrencyDashboardView: View {
    ...
    private var liveConcurrencyHeader: some View {
        HStack(spacing: 10) {
            HStack(spacing: 6) {
                Image(systemName: "brain.head.profile.fill")
                    .font(.caption)
    ...
    private var sparklineColor: Color {
        guard engine.concurrencyHistory.count >= 2 else { return .cyan }
            ...
            List {
                ...
                Section {
                    if engine.decisions.isEmpty {
                        Text("No decisions yet — AI is warming up")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    } else {
                    ...
                    Text("AI Decisions (\(engine.decisions.count))")
                }
            }
            .listStyle(.insetGrouped)
            .navigationTitle("AI Concurrency History")
